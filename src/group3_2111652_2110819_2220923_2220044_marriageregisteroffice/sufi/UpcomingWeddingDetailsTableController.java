 ...
 * @author User
 */
public class UpcomingWeddingDetailsTableController implements Initializable {
    ...
    void init(String a, String b, String c) throws IOException {
       ...
       verifyRecordTable.setItems(w.getMarriage(witnessID));
       verifyRecordTable.setRowFactory(tv -> {
           ...
           row.setOnMouseClicked(event -> {
               if (!row.isEmpty() && event.getButton() == MouseButton.PRIMARY && event.getClickCount() == 1) {
                   ...
                   try {
                       w.upcomingWeddingDetailsScene(selectedMarriage, event);
